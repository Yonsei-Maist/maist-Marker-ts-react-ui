...
import { Addon } from '../../provider/AddonProvider';
...
function LongPressToggleButton({ value, options, toolType, onClickOption }: LongPressToggleButtonProps) {
    ...
    const handleMouseUp = (event: MouseEvent<HTMLButtonElement> | TouchEvent<HTMLButtonElement>) => {
        ...
        if (event.type === 'mouseup' || event.type === 'touchend') {
            onClickOption(selectedAddon.id);
        }
    };
    ...
    return (
        <ClickAwayListener onClickAway={handleClickAway}>
            <Box sx={{ display: 'flex' }}>
                ...
                >
                    {toolType == selectedAddon.id ? selectedAddon.selectedIcon : selectedAddon.unselectedIcon}
                </ToggleButton>
                {
                    options &&
