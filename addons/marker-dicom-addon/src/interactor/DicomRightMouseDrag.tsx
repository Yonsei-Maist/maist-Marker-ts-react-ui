...
import { DicomObject } from "../reader/dicomReader";
import { DRAW_OBJECT } from '@yonsei-maist/react-maist-marker';
...
class DicomRightMouseDrag extends PointerInteraction {
    ...
    handleUpEvent(evt: MapBrowserEvent<UIEvent>) {
        const map = evt.map;
        const dicomObject = map.get(DRAW_OBJECT);
        this.clicked = false;
    ...
    handleDragEvent(evt: MapBrowserEvent<UIEvent>) {
        ...
        if (this.clicked && this.coordinate) {
            const map = evt.map;
            const dicomObject = map.get(DRAW_OBJECT) as DicomObject;
